...
void QR_factorize(int m, int n, double * A, double * tau, int p, int rank)
{
	...
	for (int i = 0; i < n; ++i) {
		...
		if (rank == root_rank) {
			...
			double anorm = norm(slice - (i % slice), &A[i % slice  + i * slice]) * norm(slice - i, &A[i % slice  + i * slice]);
			...
			for(int j = (i + 1) % slice; j < slice; j++) {
				A[i * slice + j] /= (aii - anorm);
			}

			A[i + i * slice] = 1;
            // if (i == 0) multiply_householder(slice, n-i-1, &A[i*slice + i], tau[i], &A[(i+1) * slice + i], slice, p, rank, root_rank);
			multiply_householder(slice - (i % slice), n-i-1, &A[i*slice + i], tau[i], &A[(i+1)*slice + i], slice, p, rank, root_rank);
            A[i + i * slice] = anorm;
		}
...
void multiply_Qt(int m, int k, double * A, double * tau, double * c, int p, int rank)
{
	...
	for (int i = 0; i < k; i++) {
		...
		if (rank == root_rank) {
			double aii = A[i % slice + i * slice];
			A[i % slice + i * slice] = 1;
            // if (i == 0) multiply_householder(slice, 1, &A[i % slice + i * slice], tau[i], &c[i], slice, p, rank, root_rank);
			multiply_householder(slice - (i % slice), 1, &A[i % slice + i * slice], tau[i], &c[i], slice, p, rank, root_rank);
